...
for eleType in eleTypes:
    for det in ['EB','EE']:
        histos['h_'+eleType+'_'+det+'_pt']            = ROOT.TH1F('h_'+eleType+'_'+det+'_pt', 'Electron p_{T}', 190, 10.0, 200.0)
        histos['h_'+eleType+'_'+det+'_eta']           = ROOT.TH1F('h_'+eleType+'_'+det+'_eta', 'Electron #eta', 50, 0., 2.5)
        histos['h_'+eleType+'_'+det+'_phi']           = ROOT.TH1F('h_'+eleType+'_'+det+'_phi', 'Electron #phi', 100, -M.pi, M.pi)
        histos['h_'+eleType+'_'+det+'_sigmaIEtaIEta'] = ROOT.TH1F('h_'+eleType+'_'+det+'_sigmaIEtaIEta', 'Electron #sigma_{i#eta i#eta}', 100, 0.0, 0.1)
        histos['h_'+eleType+'_'+det+'_eleID']         = ROOT.TH1F('h_'+eleType+'_'+det+'_eleID', 'Electron ID (cut based)', 10, -0.5, 9.5)
        histos['h_'+eleType+'_'+det+'_eleIDMVA']      = ROOT.TH1F('h_'+eleType+'_'+det+'_eleIDMVA', 'Electron ID (MVA)', 100, -1., 1.)
        histos['h_'+eleType+'_'+det+'_pt'+'_eleID']   = ROOT.TH1F('h_'+eleType+'_'+det+'_pt'+'_eleID', 'Electron p_{T}', 190, 10.0, 200.0)
        histos['h_'+eleType+'_'+det+'_eta'+'_eleID']  = ROOT.TH1F('h_'+eleType+'_'+det+'_eta'+'_eleID', 'Electron #eta', 50, 0., 2.5)
        histos['h_'+eleType+'_'+det+'_phi'+'_eleID']  = ROOT.TH1F('h_'+eleType+'_'+det+'_phi'+'_eleID', 'Electron #phi', 100, -M.pi, M.pi)
...
for j_entry in range(min(max_entries,n_entries) if max_entries>0 else n_entries):
    ...
    for i in range(chain_in.nEle):
        ...
        if (not ele_det == 'OA'):
            ...
            histos['h_'+ele_type+'_'+ele_det+'_eta'].Fill(chain_in.eleSCEta[i]) #use SCEta for efficiency studies
        ...
        if ( #apply a simple and very loose preselection 
            ...
            if (eleID(chain_in.eleIDbit[i])>1): # pass cutBasedElectronID-Fall17-94X-V1-loose
                histos['h_'+ele_type+'_'+ele_det+'_pt'+'_eleID'].Fill(chain_in.elePt[i])
                histos['h_'+ele_type+'_'+ele_det+'_eta'+'_eleID'].Fill(chain_in.eleSCEta[i])
